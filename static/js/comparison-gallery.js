...
// Keep generated preview URLs in sync with gallery releases so browsers and
// reverse proxies do not retain previews from an older asset layout.
const imageAssetVersion = "20260723a";
const galleryConfig = {
    groups: {
        multipart: {
...
function columnsFromVariants(variants) {
    return (variants || []).map(variant => {
        const column = {
            ...
            src: `${variant === "raw" ? "input/raw.png" : (variant === "target" ? "input/target.png" : `${variant}/preview.png`)}?v=${imageAssetVersion}`
        };
        return column;
    });
}
